...
export const MessageInput = ({
  ...
  return (
    <form onSubmit={handleSend} className="border-t border-border/45 bg-card/75 backdrop-blur-md p-3 md:p-4 flex flex-col gap-2 relative">
      ...
      {file && (
        <div className="flex items-center justify-between gap-3 p-2.5 bg-slate-900/50 border border-border/20 rounded-xl max-w-sm animate-in slide-in-from-bottom-2 duration-200">
          <div className="flex items-center gap-2 min-w-0">
            <div className="p-2 bg-slate-800 rounded-lg text-primary shrink-0">
          ...
          <button
            ...
            className="p-1 hover:bg-slate-800 rounded-lg text-muted-foreground hover:text-foreground transition-colors shrink-0"
      ...
      <div className="flex items-end gap-2 md:gap-3">
        ...
        <button
          ...
          disabled={isBlocked || isUploading}
          className={cn(
            "p-3 rounded-xl bg-slate-850 hover:bg-slate-800 text-slate-400 hover:text-foreground border border-border/30 hover:border-border transition-all duration-200 cursor-pointer",
        ...
        <div className="flex-1 relative">
          <textarea
            ...
            disabled={isBlocked || isUploading}
            rows={1}
            className="w-full min-h-[44px] max-h-[120px] py-2.5 px-4 bg-slate-905 border border-border/30 rounded-xl outline-none focus:border-primary text-[14.5px] text-foreground placeholder-muted-foreground/60 transition-colors resize-none overflow-y-auto"
            style={{ height: "44px" }}
          />
        </div>
        ...
        <button
          ...
          className={cn(
            "p-3 rounded-xl bg-primary hover:bg-primary/95 text-white font-medium shadow-sm transition-all duration-200 flex items-center justify-center cursor-pointer",
